...
enum CheckResult {
    ...
    ChainingFailure(Vec<Domino>),
    // the one on the left side of the next
    LengthMismatch(Vec<Domino>),
    // different dominoes are used in input and output
    DominoMismatch(Vec<Domino>),
}
...
fn check(input: &[Domino]) -> CheckResult {
    ...
    let mut input_sorted =
        input.iter().map(|&d| normalize(d)).collect::<Vec<Domino>>();
    ...
    // This essentially puts the first element after the last one, thereby
    // making it easy to check whether the domino chains "wraps around".
